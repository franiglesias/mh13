...
class Router
{
     ...
     *
     * $options offers three 'special' keys. `pass`, `persist` and `routeClass` have special meaning in the $options
     * array.
     *
    ...
    function getArgs($args, $options = array())
    {
        ...
        foreach ($args as $param) {
            ...
            if ((!isset($options['named']) || !empty($options['named'])) && $separatorIsPresent) {
                ...
                $passIt = (!$hasRule && !$greedy) || ($hasRule && !$self->matchNamed(
                            $key,
                            $val,
                            $rules[$key],
                            $context
    ...
    function matchNamed($param, $val, $rule, $context = array())
    {
        ...
        $controllerMatches = !isset($rule['controller'], $context['controller']) || in_array(
                $context['controller'],
                (array)$rule['controller']
            );
        if (!$controllerMatches) {
            return false;
        }
        $actionMatches = !isset($rule['action'], $context['action']) || in_array(
                $context['action'],
                (array)$rule['action']
            );
        if (!$actionMatches) {
    ...
    function reverse($params)
    {
        ...
        $url = $params['url'];
        unset(
            $params['pass'], $params['named'], $params['paging'], $params['models'], $params['url'], $url['url'], $params['autoRender'], $params['bare'], $params['requested'], $params['return']
        );
    ...
    static public function url($url = null, $full = false)
    {
        ...
        if (is_array($url)) {
            ...
            if (((strpos($url, '://')) !== false || (strpos($url, 'javascript:') === 0) || (strpos(
                            $url,
                            'mailto:'
                        ) === 0)) || (!strncmp($url, '#', 1))
            ) {

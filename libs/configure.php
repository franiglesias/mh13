...
class Configure extends Object
{
        ...
        if ($boot) {
            ...
            if (Configure::read('Cache.disable') !== true) {
                ...
                if (empty($cache['settings'])) {
                    trigger_error(
                        __('Cache not configured properly. Please check Cache::config(); in APP/config/core.php', true),
                        E_USER_WARNING
                    );
     ...
     * @access public
     */
    static public function read($var = 'debug')
    {
        $_this = Configure::getInstance();
    ...
    function load($fileName)
    {
        ...
        if (!isset($config)) {
            trigger_error(
                sprintf(__('Configure::load() - no variable $config found in %s.php', true), $fileName),
                E_USER_WARNING
            );
        ...
        switch ($load) {
            case 'model':
                ...
                if ($plugin) {
                    if (!class_exists($plugin.'AppModel')) {
                        App::import(
                            $type,
                            $plugin.'.'.$plugin.'AppModel',
                            false,
                            array(),
                            $pluginPath.DS.$pluginPath.'_app_model.php'
                        );
                    }
            ...
            case 'controller':
                ...
                if ($plugin) {
                    App::import(
                        $type,
                        $plugin.'.'.$plugin.'AppController',
                        false,
                        array(),
                        $pluginPath.DS.$pluginPath.'_app_controller.php'
                    );

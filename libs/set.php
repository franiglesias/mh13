...
class Set
{
     ...
     * array_merge) but does not do if for keys containing strings (unlike array_merge_recursive). See the unit test
    ...
    static function extract($path, $data = null, $options = array())
    {
        ...
        do {
            ...
            foreach ($contexts as $key => $context) {
                ...
                if ($token === '@*' && is_array($context['item'])) {
                ...
                } elseif (is_array($context['item']) && array_key_exists($token, $context['item']) && !(strval(
                            $key
                        ) === strval($token) && count($tokens) == 1 && $tokens[0] === '.')
                ) {
                    ...
                    if (!is_array($items)) {
                        ...
                        if (!is_array($current) || (is_array($current) && count($items) <= 1 && !is_numeric(
                                    $currentKey
                                ))
                        ) {
                            $items = array($items);

...
<?php if ($wannaUseSession): ?>
			$this->Session->setFlash(sprintf(__('%s was deleted.', true), '<?php echo ucfirst(strtolower($singularHumanName)); ?>'));
			$this->redirect(array('action'=>'index'));
<?php else: ?>
			$this->flash(sprintf(__('%s was deleted.', true), '<?php echo ucfirst(strtolower($singularHumanName)); ?>'), array('action' => 'index'));
<?php endif; ?>
		}
<?php if ($wannaUseSession): ?>

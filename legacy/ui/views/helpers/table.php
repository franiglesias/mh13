 ...
 */
    ...
    protected function _buildSelectionForm($code)
    {
        $f = $this->Form->create(
            ...
            [
                ...
                'onsubmit' => 'return confirm("'.__(
                        'Are you sure? This action affects several records and is no undoable.',
                        true
                    ).'");',
            ]
        );
        $selectionLabel = $this->Html->tag(
            'label',
            __('Action to perform with selection', true),
            ['for' => '_SelectionAction']
        );
        ...
        $area = $this->Html->tag('div', $fields, ['class' => 'mh-admin-widget', 'id' => 'mh-table-selection-action']);
        $f .= $area;
        $f .= $code;
        $f .= $this->Form->end();

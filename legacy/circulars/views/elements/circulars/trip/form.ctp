<div class="content" id="tabs-2">
	...
	<fieldset>
		<legend><?php __d('circulars', 'Trip details'); ?></legend>
		...
		<div class="row">
			<?php
				echo $this->FForm->date('Event.startDate', array(
					...
					'error' => array(
						'notEmpty' => sprintf(__d('circulars', 'Provide a date for this %s', true), __d('circulars', 'trip', true))
					),
					'clearable' => true,
				...
				echo $this->FForm->date('Event.endDate', array(
				...
				echo $this->Form->input('Event.continuous', array(
					'type' => 'hidden',
					'value' => 1,
				));
		...
		<div class="row">
			<?php echo $this->Multilingual->element('circulars/trip/event', array('plugin' => 'circulars')); ?>
		</div>

<?php
class CircularBoxesController extends CircularsAppController {

	var $name = 'CircularBoxes';
	var $layout = 'backend';

	function index() {
		
		$this->CircularBox->recursive = 0;
		$this->set('circularBoxes', $this->paginate());
	}
	...
	public function edit($id = null) {
		...
		if(empty($this->data['CircularBox'])) { // 1st pass
			if ($id) {
				$this->refreshModel($id);
			}
	...
	private function refreshModel($id)
	{
		...
		if (!($this->data = $this->CircularBox->read(null, $id))) {
			$this->message('error');

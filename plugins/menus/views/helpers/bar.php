...
class BarHelper extends AppHelper {
	 ...
	 * @access protected
	 */
	var $helpers = array('Html', 'Form', 'Menus.Menus', 'Menus.Menu');
	
	var $searchUrl = array('plugin' => 'contents', 'controller' => 'items', 'action' => 'search');
	...
	public function render($bar, $options = array())
	{
		...
		$code = $this->Html->tag('ul', $this->buildMenuList($bar), array('class' => 'left'));
		if (!empty($options['search'])) {
			$code .= $this->Html->tag('ul', $this->searchForm(), array('class' => 'right'));
		}
		return $code;
	}

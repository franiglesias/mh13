...
class Channel extends ContentsAppModel
{
    ...
    {
        if ($state === 'before') {
            $conditions = $this->manageChannelsForUser($query);
            if (!empty($query['slug'])) {
                $conditions['Channel.slug'] = $query['slug'];
            }
     ...
     * @return array
     */
    protected function manageChannelsForUser($query)
    {
        if (empty($query['user'])) {
            return ['Channel.private' => 0];
        }
    ...
    {
        return Set::combine(
            $this->Behaviors->Ownable->notOwners(
                $this,
                'User',
                array(
                    'conditions' => array(
    ...
    {
        if (preg_match(
            '/[[:xdigit:]]{8,8}-[[:xdigit:]]{4,4}-[[:xdigit:]]{4,4}-[[:xdigit:]]{4,4}-[[:xdigit:]]{12,12}/',
            $slug
        )) {
            return true;
        }

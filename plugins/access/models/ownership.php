 ...
 */	
	public function unbind($owner, $object)
	{
		...
		if ($result) {
			$this->log(sprintf('%s::%s unbound from %s::%s', $owner['model'], $owner['id'], $object['model'], $object['id']), 'access');
			return true;
		}
		
		$this->log(sprintf('Failed to unbind %s::%s from %s::%s', $owner['model'], $owner['id'], $object['model'], $object['id']), 'access');
		return false;
	}
 ...
 */	
	public function rebind($owner, $object, $access)
	{
		...
		if ($result) {
			$this->log(sprintf('%s::%s rebound with %s::%s with access %s', $owner['model'], $owner['id'], $object['model'], $object['id'], $access), 'access');
			return true;
		}
